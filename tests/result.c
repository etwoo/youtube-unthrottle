...
print_to_str_each_enum_value(void) // NOLINT(*-function-size)
{
	...
	ASSERT_IN(make(ERR_JS_PARSE_JSON_ELEM_QUALITY), CANNOT_GET);
	...
	ASSERT_IN(make_s(ERR_JS_MAKE_INNERTUBE_JSON_ID), CANNOT_FIND);
	...
	ASSERT_IN(make_s(ERR_JS_CALL_LOOKUP),
	          "Error in JS_Eval() on class expr");
	ASSERT_IN(make_s(ERR_JS_CALL_CONSTRUCTOR),
	          "Error in JS_CallConstructor");
	...
	ASSERT_IN(make(ERR_PROTOCOL_SABR_UPDATE_ALLOC), CANNOT_ALLOC);
	ASSERT_IN(make_n(ERR_PROTOCOL_HEADER_ID_OVERFLOW),
	          "Header ID is unexpectedly large");
	...
	ASSERT_IN(make(ERR_PROTOCOL_UNPACK_SABR_UPDATE), CANNOT_UNPACK);
	...
	ASSERT_IN(make_n(ERR_URL_DOWNLOAD_SET_VERBOSE), CANNOT_SET);
	...
	ASSERT_IN(make_n(ERR_URL_DOWNLOAD_SET_OPT_POST_BODY_SIZE), CANNOT_SET);
	ASSERT_IN(make_n(ERR_URL_DOWNLOAD_SET_OPT_POST_BODY_SIZE), CANNOT_SET);
	ASSERT_IN(make_n(ERR_URL_DOWNLOAD_PERFORM), "Error performing");
	ASSERT_IN(make_n(ERR_URL_DOWNLOAD_GET_STATUS), CANNOT_GET);
	ASSERT_IN(make_ns(ERR_URL_DOWNLOAD_4XX_5XX_STATUS), "Got HTTP status");
	ASSERT_IN(make_s(ERR_YOUTUBE_STREAM_URL_INVALID), "Error parsing");
	ASSERT_IN(make(ERR_YOUTUBE_N_PARAM_QUERY_ALLOC), CANNOT_ALLOC);
	ASSERT_IN(make_s(ERR_YOUTUBE_N_PARAM_MISSING), "No n-parameter");
