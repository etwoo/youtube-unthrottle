...
print_to_str_each_enum_value(void)
{
	...
	ASSERT_IN(make(ERR_JS_PLAYBACK_CONFIG_BASE64_DECODE), "Cannot base64");
	ASSERT_IN(make(ERR_JS_PLAYBACK_CONFIG_ALLOC), CANNOT_ALLOC);
	ASSERT_IN(make(ERR_JS_PROOF_OF_ORIGIN_BASE64_DECODE), "Cannot base64");
	ASSERT_IN(make(ERR_JS_PROOF_OF_ORIGIN_ALLOC), CANNOT_ALLOC);
